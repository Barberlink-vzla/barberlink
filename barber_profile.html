...
.alert-modal-close {
   ...
   /* Pega esto al final de la etiqueta <style> en barber_profile.html */
.stat-label {
    display: flex;
    align-items: center;
    justify-content: center;
...
.services-grid {
    display: grid;
    /* CORRECCIÓN: Reducimos el ancho mínimo para dar más flexibilidad en pantallas pequeñas */
...
.service-img-upload-label {
    ...
    place-items: center;
    cursor: pointer;
    border: 2px solid var(--login-card-bg);
    transition: transform 0.2s;
}
.service-img-upload-label:hover {
    transform: scale(1.1);
}
.service-img-upload-input {
    display: none;
}
.service-details {
    flex-grow: 1;
...
.service-details .service-inputs {
    display: flex;
    gap: 10px;
}
.service-details .service-inputs input {
    width: 100%;
    padding: 8px;
...
/* ==================================================================== */
/* === INICIO: ESTILOS FINALES PARA SERVICIOS (CIRCULAR Y CON TOGGLE) === */
/* ==================================================================== */

/* Contenedor principal que agrupa todo */
.profile-section-card .services-container-wrapper {
...
/* --- CLAVE PARA EL FILTRO DE SERVICIO INACTIVO --- */
.service-item-with-image.inactive {
    opacity: 0.6;
    background: linear-gradient(rgba(42, 47, 60, 0.8), rgba(42, 47, 60, 0.8)), var(--login-card-bg);
}
.service-item-with-image.inactive .service-img-preview {
    filter: grayscale(80%);
}
...
/* Contenedor de la imagen (AHORA SÍ, 100% CIRCULAR) */
.service-image-container {
    position: relative;
    width: 150px; 
    height: 150px;
    border-radius: 50%;
    overflow: hidden;  /* <-- ESTA ES LA LÍNEA CLAVE PARA RECORTAR LAS ESQUINAS */
    background-color: var(--container-bg);
...
.service-img-upload-label:hover { transform: scale(1.15) rotate(10deg); }
.service-img-upload-label .fas.fa-camera { font-size: 16px; }
.service-img-upload-input { display: none; }
...
.remove-custom-service:hover { background-color: var(--danger-color); }


/* --- CSS PARA EL INTERRUPTOR (TOGGLE SWITCH) --- */
...
input:checked + .slider {
    background-color: var(--success-color); /* Color verde para activo */
}
...
/* ================================================================ */
/* === FIN: ESTILOS FINALES PARA SERVICIOS (CIRCULAR Y CON TOGGLE) === */
/* ================================================================ */
/* --- INICIO: Estilos para las etiquetas de Precio y Duración --- */
.service-details .service-inputs {
    display: flex;
    gap: 12px;
...
.input-with-label {
    ...
    flex-grow: 1; /* Permite que ambos campos ocupen el espacio disponible */
}
...
.service-details .service-inputs input {
    width: 100%; /* El input ocupa todo el ancho de su contenedor */
    box-sizing: border-box; /* Asegura que el padding no afecte el ancho total */
}
/* --- FIN: Estilos para las etiquetas de Precio y Duración --- */
...
<div class="alert-modal-overlay" id="appointment-alert-overlay">
    ...
    <div class="alert-modal" id="payment-modal">
        ...
        <p>Registra el pago para la cita de <strong id="payment-client-name"></strong>.</p>
        <p>Monto de Referencia: <strong id="payment-amount"></strong>.</p>
        ...
        <form id="payment-form" novalidate>
            ...
            <div id="cash-payment-options" class="modal-button-group" style="display: none; margin-top: 15px;">
                <p><strong>¿En qué moneda se recibió el efectivo?</strong></p>
                <button type="button" id="pay-cash-usd-btn" class="style-button confirm-btn">
                    <i class="fas fa-dollar-sign"></i> Recibido en USD
                </button>
                <button type="button" id="pay-cash-ves-btn" class="style-button" style="background-color: var(--login-primary-accent);">
                    <i class="fas fa-money-bill-wave"></i> Recibido en VES
                </button>
            </div>
        
            <button type="submit" id="save-payment-btn" class="style-button confirm-btn" style="width: 100%; margin-top: 20px; display: none;">
                <i class="fas fa-save"></i> Registrar Pago y Finalizar Cita
            </button>
...
<button type="submit" id="save-payment-btn" class="style-button confirm-btn" style="width: 100%; margin-top: 20px; display: none;">
    ...
    <div class="alert-modal-overlay" id="password-confirm-modal-overlay">
    <div class="alert-modal" id="password-confirm-modal">
        <button class="alert-modal-close" id="password-confirm-close-btn">&times;</button>
        <h3 id="password-confirm-title">Confirmar Acción</h3>
        <p>Para continuar, por favor ingresa tu contraseña. Esta es una medida de seguridad.</p>

        <div class="input-group" style="margin-top: 20px;">
            <i class="fas fa-lock"></i>
            <input type="password" id="password-confirm-input" placeholder=" " required>
            <label for="password-confirm-input">Contraseña de tu cuenta</label>
        </div>

        <div id="password-confirm-status" class="status-message" style="margin-top: 15px;"></div>

        <div class="modal-button-group">
            <button id="password-confirm-action-btn" class="style-button confirm-btn">
                <i class="fas fa-check"></i> Confirmar y Borrar
            </button>
        </div>
    </div>
</div>
    ...
    <div class="stat-value-container">
        <div class="stat-value" id="stat-monthly-income-usd" style="display: block;">USD 0.00</div>
        <div class="stat-value" id="stat-monthly-income-ves" style="display: none;">VES 0.00</div>
    </div>

    <div class="stat-label">
        Ingresos (Mes Actual)
        <button id="toggle-income-currency-btn" class="currency-toggle-btn" title="Cambiar moneda">
   ...
   <div class="stat-chart-main-value">
    <div id="income-stat-value" class="stat-value" style="display: block;">$0.00</div>
    <div id="income-stat-value-ves" class="stat-value" style="display: none;">Bs 0.00</div>
    <div class="stat-label">Ingresos</div>
</div>
    <div style="display: flex; align-items: center; gap: 10px;">
        <div id="income-percentage" class="stat-chart-percentage neutral">
            </div>
        <button id="toggle-report-currency-btn" class="currency-toggle-btn" title="Cambiar moneda">
            <i class="fas fa-sync-alt"></i>
        </button>
    </div>
</div>
            ...
            <section id="configuracion" class="content-section">
                ...
                <div class="profile-section-card">
    <h3 class="profile-section-title" style="font-size: 1.5em; border: none;">Configuración de Moneda</h3>
    <p>Establece un porcentaje adicional para sumarle a la tasa de cambio oficial del BCV al calcular los precios en Bolívares.</p>
    <div class="input-group">
        <i class="fas fa-percentage"></i>
        <input type="number" id="tasa-markup" name="tasa-markup" placeholder=" " step="0.5" min="0" value="0">
        <label for="tasa-markup">Porcentaje Adicional sobre la Tasa (%)</label>
    </div>
    <p class="availability-instructions">Ejemplo: Si la tasa BCV es 36.50 y pones 5%, el cálculo se hará con 38.33.</p>
</div>
                <div class="profile-section-card">
    ...
    <div class="services-container-wrapper">
        ...
        <div id="standard-services-grid" class="services-grid">
            <p>Cargando servicios estándar...</p>
        </div>

        <h3 style="margin-top: 20px;">Mis Servicios Personalizados</h3>
        <div id="custom-services-grid" class="services-grid">
            <p>No has añadido servicios personalizados.</p>
        </div>
    </div>
    ...
    <h3 class="profile-section-title" style="font-size: 1.5em; border: none; color: var(--danger-color);">
        <i class="fas fa-exclamation-triangle"></i> Zona de Peligro
    </h3>
    <p>Las acciones en esta sección son permanentes y no se pueden deshacer.</p>
    <button id="delete-history-btn" class="profile-action-btn danger-btn">
        <i class="fas fa-trash-alt"></i> Borrar Historial de Citas
    </button>
    <p class="availability-instructions">
        Esto eliminará permanentemente todas las citas (completadas, pendientes, etc.) y las notificaciones asociadas. Tu perfil, clientes y configuración de servicios no se verán afectados.
    </p>
</div>
