...
// EN: js/barberProfile.js (cerca del inicio del archivo)
let currentReportCurrency = 'USD'; // 'USD' o 'VES'
...
async function loadDashboardStats() {
    ...
    try {
        ...
        const [
            ...
            // La consulta ahora trae las nuevas columnas
            ...
            supabaseClient
                .from('citas')
                .select('monto_recibido_usd, monto_recibido_ves') // <-- ¡CAMBIO CLAVE!
                .eq('barbero_id', currentUserId)
                .gte('fecha_cita', firstDayOfMonth)
                .lte('fecha_cita', today)
                .eq('estado', APPOINTMENT_STATUS.COMPLETED)
        ...
        // Calculamos ambos totales usando reduce
        const totalIncomeUSD = (monthlyIncomeData || []).reduce((sum, item) => sum + (item.monto_recibido_usd || 0), 0);
        const totalIncomeVES = (monthlyIncomeData || []).reduce((sum, item) => sum + (item.monto_recibido_ves || 0), 0);
        ...
        // Asignamos los valores a los elementos correctos del HTML
        const usdEl = document.getElementById('stat-monthly-income-usd');
        const vesEl = document.getElementById('stat-monthly-income-ves');
        
        if(usdEl) usdEl.textContent = `$${totalIncomeUSD.toFixed(2)}`;
        if(vesEl) vesEl.textContent = `VES ${totalIncomeVES.toFixed(2)}`;
...
// EN: js/barberProfile.js
// REEMPLAZA la función loadReportData por completo con esta:
...
async function loadReportData(period = 'week') {
    ...
    try {
        ...
        const [
            ...
            previousTransactionsRes, // <--- CAMBIO: Ahora traemos las transacciones completas del periodo anterior
            currentAppointmentsRes, 
            previousAppointmentsRes,
            currentClientsRes, 
            previousClientsRes
        ] = await Promise.all([
            // Consulta del período actual (ya trae todo lo que necesitamos)
            supabaseClient.from('citas')
                .select('*, barbero_servicios(*, servicios_maestro(*))') // Traemos todo para la lista de transacciones
                .eq('barbero_id', currentUserId)
            ...
            // CAMBIO CLAVE: Traemos los montos del periodo anterior también
            supabaseClient.from('citas')
                .select('monto_recibido_usd, monto_recibido_ves') // <-- Solo necesitamos los montos
                .eq('barbero_id', currentUserId)
            ...
            // El resto de las consultas no necesitan cambios
        ...
        // --- NUEVA LÓGICA DE CÁLCULO DE INGRESOS ---
        const currentPaidTransactions = (currentTransactionsRes.data || []).filter(item => item.estado_pago === 'pagado');
        
        const incomeCurrentUSD = currentPaidTransactions.reduce((sum, item) => sum + (item.monto_recibido_usd || 0), 0);
        const incomeCurrentVES = currentPaidTransactions.reduce((sum, item) => sum + (item.monto_recibido_ves || 0), 0);

        const incomePreviousUSD = (previousTransactionsRes.data || []).reduce((sum, item) => sum + (item.monto_recibido_usd || 0), 0);
        const incomePreviousVES = (previousTransactionsRes.data || []).reduce((sum, item) => sum + (item.monto_recibido_ves || 0), 0);
        
        // El resto de los cálculos de porcentaje no cambian
        ...
        // Guardamos los nuevos datos de ingresos en el objeto reportData
        // Usaremos una variable global para almacenar estos datos y que el botón de toggle pueda acceder a ellos.
        window.currentReportIncomeData = {
            usd: { current: incomeCurrentUSD, previous: incomePreviousUSD },
            ves: { current: incomeCurrentVES, previous: incomePreviousVES }
        };
        
        const reportData = {
            period,
            income: { 
                data: currentPaidTransactions // Pasamos las transacciones pagadas para el gráfico
            },
        ...
        renderReportCharts(reportData); // Esta función también la vamos a modificar
        renderTransactionList(currentPeriodAppointments); // Y esta también
...
// EN: js/barberProfile.js
// REEMPLAZA la función renderReportCharts con esta:
...
function renderReportCharts(data) {
    ...
    // --- LÓGICA DE INGRESOS MEJORADA ---
    const incomeData = window.currentReportIncomeData || { usd: { current: 0, previous: 0 }, ves: { current: 0, previous: 0 } };
    let totalToShow, previousTotal, prefix, decimals;

    if (currentReportCurrency === 'USD') {
        totalToShow = incomeData.usd.current;
        previousTotal = incomeData.usd.previous;
        prefix = '$';
        decimals = 2;
    } else { // VES
        totalToShow = incomeData.ves.current;
        previousTotal = incomeData.ves.previous;
        prefix = 'VES ';
        decimals = 2;
    }
    ...
    if (previousTotal > 0) {
        incomePercentage = ((totalToShow - previousTotal) / previousTotal) * 100;
    } else if (totalToShow > 0) {
        incomePercentage = 100;
    }

    // El gráfico se basará siempre en los USD para mantener una escala consistente,
    // pero el total mostrado cambiará.
    const incomeChartData = groupDataForChart(data.income.data, data.period, 'fecha_cita', 'sum', 'monto_recibido_usd');

    ReportCharts.renderChart({
        chartId: 'income-area-chart',
        series: incomeChartData.series,
        categories: incomeChartData.categories,
        total: totalToShow, // <-- Muestra el total de la moneda seleccionada
        percentage: incomePercentage,
        valueElId: 'income-stat-value',
        percentageElId: 'income-percentage',
        prefix: prefix, // <-- Usa el prefijo correcto
        decimals: decimals, // <-- Usa los decimales correctos
        themeColor: 'var(--success-color)'
    });

    // --- RESTO DE LOS GRÁFICOS (Sin cambios) ---
...
// EN: js/barberProfile.js
// REEMPLAZA la función renderTransactionList
...
function renderTransactionList(appointmentsToRender) {
    ...
    listEl.innerHTML = appointmentsToRender.map(item => {
        ...
        let amountDisplay = '--'; // Valor por defecto

        if (item.estado_pago === 'pagado') {
            if ((item.monto_recibido_usd || 0) > 0) {
                amountDisplay = `+ $${item.monto_recibido_usd.toFixed(2)}`;
            } else if ((item.monto_recibido_ves || 0) > 0) {
                amountDisplay = `+ VES ${item.monto_recibido_ves.toFixed(2)}`;
            }
        } else if (isDebt) {
            // Las deudas se muestran con el valor referencial en USD
            amountDisplay = `- $${(item.precio_final || 0).toFixed(2)}`;
        }
...
if (toggleReportBtn) {
    toggleReportBtn.addEventListener('click', () => {
        ...
        currentReportCurrency = currentReportCurrency === 'USD' ? 'VES' : 'USD';

        // Llama a una función que solo refresca el gráfico de ingresos
        // sin tener que recargar todos los datos.
        refreshIncomeChart();
    });
}
    
    // EN: js/barberProfile.js
// PEGA esto dentro de la función setupEventListeners

const toggleBtn = document.getElementById('toggle-income-currency-btn');
if (toggleBtn) {
    toggleBtn.addEventListener('click', () => {
        const usdEl = document.getElementById('stat-monthly-income-usd');
        const vesEl = document.getElementById('stat-monthly-income-ves');
        if (usdEl && vesEl) {
            const isUsdVisible = usdEl.style.display === 'block';
            usdEl.style.display = isUsdVisible ? 'none' : 'block';
            vesEl.style.display = isUsdVisible ? 'block' : 'none';
        }
    });
}
...
// EN: js/barberProfile.js
// AÑADE esta nueva función en cualquier lugar del archivo
...
function refreshIncomeChart() {
    ...
    // Simplemente actualizamos los textos y porcentajes del DOM
